#N_estimation
# tests how lack of CMR data in some years affect posterios. For this test,
# years with no CMR data were excluded. Only years with both Tally and
# CMR data - use plot_posterior_onlyWithLP.R to plot posteriors. 
...
# Jags starts here:
 # tic <- Sys.time()
 # bugs.data.P <- list(n = LP.list$n.mat,
 #                   m = LP.list$m.mat,
 #                   M = LP.list$M.mat,
...
# tic <- Sys.time()
 bugs.data.I <- list(n = n.mat.df[LP.n>0,],
                     ...
                     T = length(uniq.yrs[LP.n>0]),
                     TC_n = TC_n[LP.n>0],
                     n_dates = as.vector(n_dates$n_dates[LP.n>0]),
                    TC = TC[LP.n>0,])
